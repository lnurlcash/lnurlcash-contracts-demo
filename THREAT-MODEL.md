...
- **Arbiter forks one contract id:** a known second offer with the same contract id is refused rather than treated as a revision.
...
- **Valid acceptance substitution:** one role cannot overwrite its first accepted event; a locally owned participant must still possess secrets matching the packet before funding.
- **Arbiter forks the bond packet:** a second valid pair of bond requests cannot replace the first canonical packet.
...
- **Outcome suppresses dispute:** an explicit dispute takes precedence over ordinary participant outcomes.
...
- **Pre-signed or conflicting judgement:** a decision must follow a signed dispute; two distinct decisions are retained as equivocation evidence and freeze settlement.
...
- **Equivocation hidden across isolated devices:** a client can fail closed only after it receives both forks. Participants still need a shared transcript or gossip transport to discover split-view attacks.
